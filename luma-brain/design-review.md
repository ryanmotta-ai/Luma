...
* **❌ Não usar gradiente em botões:** O gradiente sutil de laranja é elegante em **superfícies decorativas** (faixa da topbar, faixa de destaque no topo de modais, ícone-badge). Mas **botões/CTAs usam cor CHAPADA** (`--dm-orange`, hover `--dm-orange-d`) — sem gradiente (preferência do usuário confirmada). E jamais gradientes multicoloridos (roxo→azul, arco-íris) ou contrastes agressivos.
