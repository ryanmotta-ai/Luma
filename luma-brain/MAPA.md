...
`const`/`let` no escopo global. Sem módulos (1ª lei), dois arquivos que declaram o mesmo nome
derrubam um deles inteiro com `Identifier has already been declared` — e o `node --check` de cada
arquivo passa, porque a colisão só existe quando os dois carregam juntos. Aconteceu em 09/2026
(`_ICO_CHECK`, em `chat.js` e `core/user-profile.js`). Ao criar `const` de topo, `grep -rn "^const NOME" js`.
...
**Tamanho real de hoje:** 79 arquivos JS (69.796 linhas, 2.619 funções) · 31 arquivos CSS (29.238 linhas) · `index.html` com 3.884 linhas e 80 `<script>`.

## JS — o que cada arquivo é

### js (raiz)

**`js/00-config.js`** · 3854 linhas
...
· API: gVarRegex, gValidVarName, gXmlEsc, gRoundPolyD, gRoundPolyPath2D, gVectorPathFillRule, gVectorPathValid, gTraceVectorPath, gVectorPathD, gFxOffset, gFxRgba, gGradStopsCss, gGradientCss, gGradientCanvas … (+67; 112 funções no total)
...
Schemas e Validadores Determinísticos da Camada de IA. Regra §7 e §62: "GEMINI SUGERE. LUMA VALIDA. HUMANO DECIDE." Output estruturado pelo modelo NÃO significa semanticamente correto. Todo dado retornado é auditado e…
...
**`js/core/auto-layout.js`** · 4821 linhas
AUTO-LAYOUT — a camada de JULGAMENTO O solver de composição mora em `00-config.js` (`gApplyRelativeAnchors`):
· API: gLayoutFontProbe, gStampLayoutBaseline, gLayoutLimpaCarimbos, gLayoutTextoAutorado, gEnsureLayoutBaseline, gLayoutFontDrift, gLayoutFontStatus, gLayoutRefInk, gLayoutRoleOf, gLayoutSemanticRole, gCompileLayoutRoles, gLayoutRoleMaxLines, gLayoutCampoEhPreco, gLayoutEhPrecoDinamico … (+77; 120 funções no total)
...
**`js/core/supabase.js`** · 177 linhas
...
**`js/franqueado/catalog.js`** · 1585 linhas
...
· API: fSwitchTab, fSetHistFilter, fGoToCampaigns, fAskClearHist, fHistVoltar, fRenderHist, fDownloadHist, fEditFromHist, fDuplicateInOtherFmt, fConfirmDuplicate, fEditCampFolder, fCampAdminMenu, fCampAnalyticsClose, fCampAnalyticsOpen … (+33; 81 funções no total)
...
**`js/franqueado/live-preview.js`** · 2913 linhas
...
**`js/franqueado/upload-panel.js`** · 256 linhas
...
**`js/designer/publish.js`** · 1531 linhas
...
· Estado global: dPubSelectedABs, dPubPermissoes, dPubObservers, dPrevToolForSpace, dPubWizardStep, dPubLinterStats, dPubLastTrigger, dPubDraftTimer, dPubPublished, dDirty (+4)
