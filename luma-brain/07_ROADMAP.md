...
- [ ] ⚠ **`layoutRole` é lido e nunca escrito** — nenhuma UI ou importador marca uma camada como `protected`/`background`; quem protege de fato é `locked`/`lockPosition`. O solver genérico que dependia de `protectedAreas` foi removido em 2026-08-13; decidir se `layoutRole` vira metadado compilado pelo importador ou se o conceito remanescente sai do runtime.
