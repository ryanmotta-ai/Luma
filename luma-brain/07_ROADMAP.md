...
- [x] ⚠ **`layoutRole` é lido e nunca escrito** — **RESOLVIDO em 2026-08-14 pelo caminho "metadado compilado"**: `gCompileLayoutRoles` (`js/core/auto-layout.js`) classifica sozinho na importação, no vínculo e no solver. `layoutRole` mantém o vocabulário antigo do runtime (`background`/`protected`, as duas palavras que a cascata já lia) e a classificação rica vai para `layoutSemantic`. Campo dinâmico nunca é carimbado — esses papéis imobilizam a camada. Detalhe no `docs/LUMA.md` §Auto-layout.
...
- [x] **Layout vivo: a arte reacomoda com o texto do franqueado (2026-08-05).** O pedido: digitar no chat e ver o template se adaptar — bateu no respiro, quebra linha; o de baixo desce; conforme o caso, encolhe. **Fases 0 e 4 entregues.** A cascata (`gApplyRelativeAnchors`) já existia e já rodava no render do franqueado — estava sendo alimentada com altura errada, porque medir e desenhar eram caminhos separados (detalhe em `docs/LUMA.md`). Agora há `gFitTextLayer`, a única resposta para "como este texto ocupa esta caixa", usada pelos dois lados; refactor **pixel-idêntico** em 16 casos. O interruptor tem dois níveis em série (flag da rede + `publishMeta.layoutVivo`, com toggle no publicar), padrão desligado. Custo medido: 12ms/render com 30 camadas. **Fase 3 entregue (2026-08-05):** as correntes são inferidas do próprio desenho — bloco alinhado logo abaixo de outro e a menos de duas linhas de distância vira filho dele; fundo, protegida, travada e filha de grupo ficam fora; âncora manual vence. A régua do gap é a CAIXA desenhada e a corrente só empurra: cabe na caixa → arte idêntica à publicada; passa → desce o excedente. *(A alternativa — medir um estado de referência com os valores de exemplo — foi descartada: dependeria do `dVars`, que pode estar vazio na sessão do franqueado, e referência diferente entre Estúdio e franqueado é a divergência de sempre.)* **Fases 1 e 2 entregues (2026-08-06):** a escada é **quebrar → empurrar → encolher**, nessa ordem, com dois pisos. O piso da hierarquia (`gStampPisosHierarquia`) impede que "diminuir" inverta a arte: uma camada nunca desce abaixo do degrau tipográfico imediatamente menor (título de 92 com preço de 64 para em 64), e o piso antigo de 50% continua valendo quando é mais restritivo. O laço fechou dentro da cascata: posiciona → se a tinta escapou da prancheta, reduz o teto de quem cresceu além da própria caixa (8% por passo, máx. 12) e reposiciona. Quando todo mundo chega ao piso e ainda não cabe, a prioridade inverte uma vez — arte com o preço cortado é inútil, arte com hierarquia achatada é feia mas serve — e desce até o piso absoluto. Custo: ~6ms. Três divergências medida × desenho foram achadas medindo, não lendo: **(a)** com teto imposto, `gSmartWrapText` ainda quebrava no tamanho DESENHADO — 7 linhas onde cabiam 3, encolhendo e crescendo em altura ao mesmo tempo; **(b)** sem `vAlign:'top'` o render CENTRALIZA na caixa, então o texto que passa dela transborda metade para cima — a cascata só media para baixo e o título comia a margem do topo; **(c)** o encadeamento era caixa-a-caixa e passou a ser tinta-a-tinta (`_gInkDy`), a mesma régua para âncora manual e inferida. A regra que resolveu (b): **texto não sobe** — centralizado enquanto cabe na caixa, ancorado no topo quando passa dela (`_vTopAuto`), crescendo só para baixo, que é o que a corrente sabe absorver. **Fase 6 entregue (2026-08-06):** o Estúdio avisa antes de publicar. `gStressValues` monta o pior caso PERMITIDO pelo `maxLen` — o texto mais longo que o franqueado pode digitar, nunca um a mais (testar além seria alarme falso, aquém seria não testar). O checklist ganhou "O pior caso não cabe", que monta a arte com esse texto pelo mesmo caminho do render e diz o número que o designer controla: *"com 32 caracteres em «Produto», «Título» invade «Preço»"* — e só acusa par que não se sobrepõe já no exemplo, senão o selo desenhado atrás do texto viraria alarme. De quebra: o cenário "Limite" do Simular dados reais tinha frases cravadas que ignoravam o `maxLen`, e o selo "Revisar encaixe" por campo estava no HTML sem nunca acender (faltava ligar o `_fOverflowSink` que o render já expunha). **Botão Auto-layout (2026-08-06):** a pedido do Ryan, a chave virou um botão ao lado do Auto-zoom na barra da prévia ao vivo — o franqueado pode preferir a composição original mesmo com o texto apertado, e gosto é dele. De quebra, achado ao medir: **a barra da prévia já transbordava 101px** num painel de 380px — o botão "Mockups" ficava fora da tela, sem scroll e sem pista de que existia; a barra chegou a quebrar em duas linhas e depois foi compactada para uma só, sem remover funções. E `[hidden]` não escondia nada nesses botões, porque o `display:inline-flex` da classe vence o do navegador. **Ajuste do Ryan no mesmo dia:** o designer não decide — **todo template nasce com o layout vivo ligado** e o interruptor por template saiu do publicar (`publishMeta.layoutVivo` removido do default, do modal e do render). Sobraram DOIS interruptores: a flag da rede (gestão) e o botão da prévia (franqueado). E o efeito ficou **só do lado do franqueado**: Estúdio, prévia do designer e Simular dados reais voltaram à geometria desenhada — camada escorregando sob o cursor de quem posiciona é o oposto de uma ferramenta de autoria, e o simulador serve para ver o problema, não o conserto. O checklist audita pela mesma régua (o franqueado pode desligar o Auto-layout, então a arte desenhada é a pior situação real). **Eixo X e refinos (2026-08-06):** mapeei o motor com sondas e ataquei o que apareceu. A cascata nasceu vertical, e isso deixava três buracos: **point text corria para fora da arte** (2385px de tinta numa prancheta de 1080 — ele não quebra nem encolhe, por regra do render), **não havia corrente lateral** (o "De R$ 149,90 por" cobria o preço em 211px) e a tinta horizontal era ignorada (`_gInkDx`, o irmão do `_gInkDy`). Junto: **prioridade de encolhimento por ORDEM** — o menor degrau com folga cede sozinho até parar de estourar, e só então a escada sobe; antes o título ia ao piso enquanto o regulamento jurídico parava um degrau antes (era −50%/−39%, virou −14%/−22%). **O vão do campo vazio fecha** (`anchor.colapso`, a única exceção ao "só empurra", limitada à altura exata da faixa que sumiu). **`_foraDaArte`** carimba o que a escada não conseguiu salvar. **Custo:** `_posicionar` era O(n²) e virou ordem topológica, uma passada — arte real 2,6ms, caso patológico de 40 textos 64ms contra 116ms. O checklist passou a acusar fuga nos quatro lados, com conserto específico para point text. ⛔ **Uma suspeita foi descartada medindo:** o piso da hierarquia parecia degenerar com assinatura miúda no rodapé, mas `degraus.find` devolve o maior degrau abaixo — em quatro hierarquias reais nenhuma camada pôde descer abaixo de outro texto. Código intacto. **Placa, entrelinha e barra (2026-08-07):** sondei o motor de novo e vieram três coisas. **A placa atrás do texto cresce junto** — o padrão "card" transbordava 62px e a cor saía debaixo da letra; inferida do desenho com regras apertadas (só retângulo, atrás no z-order, envolvendo o texto, ≤6× a área, um texto só), e cresce nos dois eixos — em largura só por point text, seguindo a direção da tinta. **A entrelinha virou o degrau antes de encolher a fonte** — designer fecha o espaçamento antes de diminuir a letra, e o aperto é calculado (`altura / fonte × linhas`), não tateado. **O checklist ganhou dois níveis** consumindo o `_foraDaArte`: alerta quando o Auto-layout conserta, erro quando nem ele salva. **A barra da prévia foi reorganizada:** reset/Ajustar, zoom out, **slider**, percentual, zoom in, Auto-zoom, Auto-layout e Mockups ficam na mesma linha; abaixo de 390px os rótulos automáticos viram ícones, mas todos os comandos continuam acessíveis. A arte pode ser arrastada livremente, inclusive abaixo do fit (35%) e até 600%; roda e pinça usam o mesmo motor logarítmico. De quebra, **a barra era branca no tema escuro** — o painel tinha versão escura desde sempre e a barra tinha sido esquecida. ⛔ **Duas suspeitas descartadas medindo:** a ordem da caixa-alta (hoje entrega 3 linhas contra 4 e cabe — mexer só mudaria arte publicada) e formatos divergindo (o `gReflowLayers` escala proporcional, os quatro formatos deram idêntico). **Layout vivo concluído.** **Fora de escopo por decisão do Ryan:** aviso visual na prévia do franqueado — o ajuste acontece calado.
- [x] **Auto-layout exclusivo e fail-safe do franqueado (2026-08-13).** `fRenderTemplateLayers` ganhou escopo explícito e cai em `designer` por padrão; Estúdio, SVG, publicar e Simular dados reais mantêm a composição original, o botão "Corrigir layout" e o solver genérico foram removidos. Prévia/exportação do franqueado resolvem clones temporários pelo mesmo motor. O botão pode pedir a composição original somente quando ela é segura; adaptação necessária é forçada. Sem solução, PNG/PDF/share/lote são bloqueados por `LUMA_LAYOUT_UNSAFE`. Endurecimentos: referência autorada determinística, sobreposição parcial limitada ao baseline, respiro mínimo real inclusive na borda, wrapping por parágrafo manual e grapheme, placas dentro do mesmo grupo PSD, limite semântico de linhas, sangria autorada preservada e escala final só do componente afetado. Validado em 7 PSDs reais (31 pranchetas, 588 nós, 207 textos): nominal e longo seguros, extremos impossíveis bloqueados, pior solve 56ms. Regressões browser-native: `tests/auto-layout.html` (17 cenários) e `tests/psd-import.html` (4).
- [~] **Auto-layout: de guardião de colisão a avaliador de composição (2026-08-14).** O pedido do Ryan em uma frase: *"ele é um excelente guardião de colisões; para virar o Auto-layout muito foda precisa também ser um avaliador de composição, capaz de comparar soluções e escolher a que mantém a intenção visual do designer."* Entregue nesta ordem, com o solver continuando **um só** (`gApplyRelativeAnchors`) — o arquivo novo (`js/core/auto-layout.js`) não mede nem posiciona nada, ele prepara a entrada, chama o mesmo solver com a ordem dos degraus trocada e julga por nota.

  **P0 — antes de chamar de produção sólida**
  - [x] **Baseline autorado universal.** Era só do PSD; agora nasce no vínculo (`dLayerBindField`), na importação e — para material já publicado — é **reconstruído em runtime** a partir do exemplo do campo (`gEnsureLayoutBaseline`). Sem migration, sem deploy, sem reabrir template. Guarda texto, geometria, métricas e uma sonda de fonte. O fixture `legado-sem-baseline` do corpus existe só para cobrar essa migração.
  - [~] **Determinismo de fontes entre plataformas.** A causa foi atacada: a decisão do solver passa a ser calibrada pelo desvio de métrica (`gLayoutFontDrift`), então fonte carregada, ausente ou substituída produzem o MESMO veredito. Coberto por teste com fonte substituída. ⚠ **Falta exercer em Safari/iOS e Android reais** — não dá para fazer daqui, e a promessa honesta é "decisão igual", não "pixel igual" (rasterizadores diferentes desenham diferente).
  - [x] **Corpus reproduzível no CI.** `node scripts/run-browser-tests.js` roda as 4 suítes num Chromium headless **sem dependência nova** (DevTools Protocol direto, WebSocket nativo do Node 22), e `.github/workflows/tests.yml` faz disso portão de push/PR. O corpus são 5 pranchetas sanitizadas × 3 cenários, com invariantes independentes de fonte + golden de geometria e **assinatura visual (dHash)** ancorado na pilha de fontes de quem gravou (pilha diferente → aviso, nunca falso vermelho). 99 casos verdes.
  - [x] **Telemetria de produção.** Evento `layout_resolvido` com veredito, campo culpado, estratégia vencedora, nota, tempo, voltas, template/material/formato e estado das fontes. Trava por chave na prévia (que re-renderiza a cada tecla); exportação sempre registra. Nunca vai conteúdo do franqueado, só o nome do campo.

  **P1 — qualidade de designer**
  - [x] **Compilador semântico invisível** — o `layoutRole` do §6, sem nenhum passo a mais para o designer.
  - [x] **Gerar e comparar alternativas** — três políticas concorrentes (`sem-entrelinha`, `entrelinha-livre`, `proporcional`) + a padrão; ganha a de menor penalidade. Determinístico e condicional: só roda quando a escada passou do empurrão, e **nunca** decide por tempo/carga (prévia e exportação têm que escolher a mesma composição).
  - [x] **Pontuação estética** — nove itens: inversão e desvio de hierarquia, corpo perdido, deslocamento, linhas extras, órfãs/conector/valor partido, respiro, densidade, alinhamento e centro de massa.
  - [x] **Safe zones de imagens** — `_dPsdInkBox` mede a caixa do assunto opaco na importação (onde os pixels ainda existem) e o solver protege o ASSUNTO, não a moldura. De quebra libera a margem transparente de PNG recortado, que antes era obstáculo à toa.
  - [x] **Quebra semântica** — `R$ 29,90`, `US$ 15`, `50 %`, `500 ml`, `2 por` e preposição órfã viram unidade única, com cola condicional (se não couber colada, não cola — quebra dura no meio da palavra seria pior).

  **P2 — operação gigante**
  - [x] **Orçamento de desempenho real** — `LUMA_CPU_THROTTLE=4` simula celular fraco no runner. A medição achou o gargalo (medida e quebra recalculadas a cada volta, `Intl.Segmenter` criado por token): p95 de 118→52ms com CPU normal e 432→180ms a 4×; pior solve 614→97ms e 2227→1076ms.
  - [x] **Fuzzing** — 60 entradas hostis com semente fixa (emoji, CJK, fullwidth, moeda, data, caixa-alta, especiais, campo vazio, três campos crescendo juntos) cobrando contrato: nunca lança, sempre termina, geometria finita, veredito válido.
  - [x] **Saída acionável quando impossível** — `gLayoutDiagnosis` diz qual campo travou e quantos caracteres cabem, achando o maior conteúdo seguro por busca binária no próprio motor. Quando a camada reprovada é vítima (CTA empurrado, rodapé atropelado), sobe pela corrente até o ancestral com campo — foi o que levou bloqueios sem diagnóstico de 11/12 para 1/12 no fuzzing.
  - [x] **Aprendizado pelo corpus** — todo PSD problemático vira arquivo em `tests/corpus/` + um `<script>`; a partir daí não regride em silêncio.

  **Três defeitos reais que o próprio trabalho achou** (medindo, não lendo): **(a)** a escala proporcional de emergência ignorava quem estava FORA do componente e deixava o produto sair menor que o preço — inversão de hierarquia comprovada em `promo-preco-circulo · extremo`; **(b)** o teto semântico de linhas encolhe a fonte DENTRO do `gFitTextLayer` sem passar por `_tetoFonte`, e o veredito saía `original` numa arte com o título reduzido de 82px para ~50px — mentia para o botão "prefiro a composição original" e para a telemetria; **(c)** os gargalos de desempenho acima.

  ⚠ **Aberto de propósito:** verificação em Safari/iOS e Android reais; o caso patológico de 1s a 4× de CPU; e a UI de `layoutRoleManual` (hoje o papel é 100% compilado — só existe UI quando alguém precisar contrariar a compilação).

- [x] **Refino do Auto-layout — medido em massa, não lido (2026-08-14).** Pedido do Ryan: "quero o Auto-layout muito sólido". O jeito de refinar foi construir o instrumento antes de opinar: `tests/_bancada.html` roda os fixtures com cada campo crescendo palavra a palavra (**276 cenários**) e reporta veredito, dano real, política vencedora, itens da nota que disparam e tempo. Fica **fora do CI** (o runner pula suítes com `_`) porque é instrumento, não portão. Os cinco achados vieram todos dela:
  - 🔴 **12 de 14 bloqueios eram FALSOS.** `estourou` misturava "não coube" com "passou do teto de linhas". Arte inteira dentro da prancheta, sem tocar em nada, era **recusada por gosto editorial** — o erro mais caro possível aqui (franqueado sem arte e sem saída). Separado em `estourou` (dano, reprova) e `excedeuLinhas` (preferência, guia a escada e pesa na nota). Bloqueios: 16% → **1%**.
  - 🔴 **O guardião não via a camada FIXA empurrada.** O varredor de colisão só olhava camadas com campo; o CTA fixo empurrado para cima da foto passava batido e **3 artes saíam aprovadas com dano**. Agora entra qualquer texto que a adaptação mexeu, e o culpado sobe a corrente até a raiz dinâmica. Bônus: enxergando antes, a escada passou a **consertar** casos que saíam estragados. Artes aprovadas com dano: **0**.
  - 🟠 **A nota media o texto do franqueado, não o motor.** Densidade e equilíbrio disparavam em 100% dos cenários — inclusive nos intocados — porque comparavam com a referência autorada. Agora há duas referências para duas perguntas: autorada para o CULPADO ("cresceu além do desenho?") e `_layoutSemAjuste` para a NOTA ("quanto a adaptação custou?"). Arte intocada pontua zero, e isso virou invariante testada.
  - 🟠 **Metade das trocas de política era ruído** (ganho de ~1 ponto em 150–390, movendo o CTA 14px). Alternativa agora precisa de margem visível para destronar a padrão: 3 pontos ou 2%. Trocas 78% → 41%; ganho mediano 1,1 → 14,6.
  - 🟠 **Fechar a entrelinha estava barato demais** (peso 13 contra 55 do corpo): a nota PREFERIA destruir o respiro. Sintoma: a política que preserva entrelinha não vencia nem no fixture criado para ela. Peso 48, proporcional à fração perdida — agora as quatro políticas ganham espaço.

  Dois testes antigos caíram na revisão e estavam **errados, não regredidos**: o "caso impossível" media 72×24 e o texto cabia em 11px dentro da prancheta — passava só pela confusão do `estourou`. Trocado por um caso que escapa 19px pelo rodapé de verdade. Corpus cresceu para 6 fixtures (novo: `bloco-arejado`, com entrelinha 1.9 como intenção de design) e a suíte para **115 casos**. ⛔ `editorial` e `alinhamento` nunca disparam em arte real e **ficam**: provados funcionando isolados, marcam zero porque a quebra semântica já evita o defeito — são guardas, não código morto.
...
- [~] **Camadas de ajuste aplicadas (2026-08-13, núcleo entregue).** Antes eram contadas e dropadas. Agora entram no modelo como `type:'adjustment'` e o renderer canônico recalcula Brilho/Contraste, Níveis, Curvas, Exposição, Vibração, Matiz/Saturação, Inverter, Posterizar e Limiar no ponto real da pilha. Respeitam grupo isolado, máscara, clipping, opacidade e blend; a prancheta do Estúdio usa o mesmo renderer cumulativo e o painel permite editar os parâmetros. QA sintético cobriu matemática, round-trip parser→layer→render, meia opacidade, máscara, clipping e escopo de grupo; o PSD real manteve 98,32% / 95,16% / 99,24%. **Resíduo:** LUT, Cor seletiva, Mapa de gradiente, Balanço de cor, Preto e branco, Filtro de foto e Mixer de canais permanecem identificados como não aplicados; implementar por valor/PSD real, sem aproximação muda.
- [~] **Pilha de efeitos múltiplos (2026-08-13, formas entregues).** Photoshop aceita 2+ sombras, contornos e overlays na mesma camada; o importador guardava só o primeiro. Formas agora carregam `layerEffects` com todas as sombras projetadas/internas, sobreposições de cor/gradiente e contornos sólidos, mantendo a ordem dentro de cada tipo; export e prancheta compõem a pilha, contornos externos ficam atrás do fill e internos na frente, e a revisão mostra quantos efeitos foram preservados. O Estilo de Camada ganhou instâncias individuais: selecionar, reordenar, duplicar, remover e editar; efeitos legados são promovidos sem perda ao duplicar, e o primeiro continua espelhado para compatibilidade com SVG/templates antigos. Gradientes mantêm stops intermediários ao editar as extremidades. QA sintético cobriu parse, composição, seleção da segunda sombra sem tocar a primeira, ordem, add/remove, promoção legada e gradiente; o PSD real ficou estável em 98,32% / 95,16% / 99,24% antes deste pacote de UI (o parser não mudou). **Resíduo:** pilha em texto/imagem continua imagem fiel com aviso; blend não normal é sinalizado como aproximação; gradiente/pattern em contorno continua sem primitiva dinâmica.
- [x] **Custom shapes/path Bézier editáveis (2026-08-13).** `vectorMask.paths` deixa de virar retângulo heurístico: âncoras e alças são normalizadas na caixa da layer e o helper único `gTraceVectorPath`/`gVectorPathD` alimenta Canvas final, Estúdio, previews, molduras, clipping vivo e SVG. A geometria sobrevive ao resize, persistência e conversão Forma↔Moldura; a revisão marca "Path Bézier · editável". QA sintético cobriu curvas cúbicas, compound path `evenodd`, normalização PSD e resize. **Limite honesto:** invert/subtract/intersect/exclude continuam como máscara raster fiel; não são promovidos enquanto não houver álgebra booleana idêntica à do Photoshop.
