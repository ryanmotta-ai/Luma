...
- [x] **Auto-layout exclusivo e fail-safe do franqueado (2026-08-13).** `fRenderTemplateLayers` ganhou escopo explícito e cai em `designer` por padrão; Estúdio, SVG, publicar e Simular dados reais mantêm a composição original, o botão "Corrigir layout" e o solver genérico foram removidos. Prévia/exportação do franqueado resolvem clones temporários pelo mesmo motor. O botão pode pedir a composição original somente quando ela é segura; adaptação necessária é forçada. Sem solução, PNG/PDF/share/lote são bloqueados por `LUMA_LAYOUT_UNSAFE`. Endurecimentos: referência autorada determinística, sobreposição parcial limitada ao baseline, respiro mínimo real, wrapping por parágrafo manual e grapheme, placas dentro do mesmo grupo PSD e escala final só do componente afetado. Regressão browser-native em `tests/auto-layout.html`: 11 cenários.
