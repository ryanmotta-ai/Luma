...
- **Painel "Mídias" / Módulo "Marca"** (mesma ideia, dois ângulos — inspiração: painel de mídias do Dashquify) — logos/assets oficiais da Delivery Much + mídias do designer, centralizados. **NÃO é módulo novo**: a Biblioteca do Studio (`biblioteca_assets`, `js/designer/library.js`, drawer "Recursos") já tem nome/categoria/tipo e sync com Supabase. **Escopo decidido (Ryan, 2026-07-18): só reorganizar o que já existe** — reagrupar a Biblioteca atual em abas **Marca** (logos/imagens oficiais) × **Minhas mídias**, reaproveitando 100% da tabela e do sync. **Fora de escopo por ora:** captura automática de uploads que hoje são descartados (PSDs importados, fotos aplicadas em moldura `canvas.js:1170`) — ficam pra depois, quando quisermos fechar esse buraco. Esforço baixo (patch em `library.js` + markup do drawer).
- **Refazer a tela de login** (`#g-login-screen`, `index.html:167-234`) — reconstruir o visual pra algo mais estético/premium, com base em referências que o Ryan vai trazer. Contrato funcional a preservar intacto: `gDoLogin`/`gDoForgot`/`gShowLoginView`/`gShowForgotView`/`gTogglePass` e os IDs `gl-email`/`gl-pass`/`gl-error`/`gl-btn-login` (+ `gf-*`) que o `auth.js` enxerga. Oportunidade: hoje é tudo `style=""` inline com hex cravado (`#FF9000`, `#e53e3e`, `#38a169`) — migrar pra tokens + classe CSS no redesign. Esperar as inspirações do Ryan antes de desenhar. Esforço médio (só front, 1–2 arquivos).
