...
10. ENTREGAR   → commit + push na talpaipai (só os arquivos da tarefa), mostrar o diff.
```
...
- ✅ **Commit e push automáticos** na `talpaipai` ao concluir a tarefa — sem pedir confirmação (decisão do usuário em 2026-09-24). Depois, mostre o que foi (`git diff --stat` + resumo). ⛔ **Nunca `git add .`** (a raiz tem arquivos pessoais/segredos): adicione só os arquivos da tarefa, pelo nome.
- ⛔ **UMA branch só: `talpaipai`.** Todo commit e todo push vão para ela. Não crie branch de feature, não use branch que uma ferramenta sugeriu, não abra PR por hábito.
  ...
  - ⛔ **`talpaipai` é produção.** Push que toque `js/`, `css/` ou `index.html` exige **subir o `?v=N`** antes (`03_ENGINEERING` §6.1). Push é deploy para a rede inteira.
...
- ❌ `git add .`, ou ação destrutiva sem confirmar.
...
- [ ] Guardrails: escape, tokens, RLS, sem segredo, **commit só com os arquivos da tarefa (sem `git add .`)**?
