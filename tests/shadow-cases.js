...
(async function(){
  ...
  test('shadow: distribuição de resultados',()=>{
    ...
    const lfMod=REGISTROS.some(r=>r.localFit&&r.localFit.disponivel);
    avisos.push('   Local Fit: 0 (0%) — o módulo '+(lfMod?'existe mas não está ligado a'
      :'não existe em')+' nenhum caminho de produção; o proxy medido é original-first, acima');
  ...
  test('shadow: desempenho por etapa e por porte de arte',()=>{
    ...
    avisos.push('   Local Fit: '+(REGISTROS.some(r=>r.localFit&&r.localFit.disponivel)
      ? 'módulo carregado, fora do caminho — nada a medir' : 'não existe nesta base — nada a medir'));
