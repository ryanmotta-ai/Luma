...
**Correntes inferidas do desenho** (`_gInferirCorrentes`, só com o layout vivo ligado). `relativeAnchor` existe há tempos mas é marcado camada a camada, à mão — por isso a cascata quase nunca entrava em ação numa arte real. Agora a corrente sai do próprio desenho: bloco alinhado logo abaixo de outro (cruzamento horizontal ≥30% da faixa mais estreita) e a menos de **duas linhas** de distância (na escala tipográfica da própria arte) vira filho dele; o vizinho imediato acima ganha a paternidade. Ficam de fora: fundo, travada, com posição travada, o próprio grupo e **filha de grupo TRAVADO/protegido** — é assim que logo e selo não são empurrados por texto, e o caminho real é **travar** a camada. ⚠ **Corrigido em 13/08:** a regra barrava *qualquer* camada com `parentId`. Fazia sentido quando grupo só nascia à mão no Estúdio; desde a importação de PSD com grupos, quase toda camada de arte de agência tem pai — e a regra desligava a corrente inferida do template inteiro, em silêncio. Agora o pai é resolvido por ID e só imobiliza se ele mesmo estiver travado/protegido. ⚠ `layoutRole` é lido mas **nenhuma UI o escreve** (6 leituras, 0 escritas — varredura pró-1.0); fica como reserva. **Âncora manual do designer sempre vence a inferida.**
...
**Corredores de composição e respiro.** `_gLayoutBaseVisual` registra a geometria **autorada** como referência antes de qualquer movimento — no PSD, `layoutRefText` guarda o texto usado pelo designer e o mede com a mesma fonte disponível no navegador, calibrando substituições sem depender de texto de exemplo/`dVars`. Se título e obstáculo estavam separados, existe um respiro mínimo (0,8% do lado curto ou 18% do corpo, limitado pela escala da arte); a própria borda da prancheta também é obstáculo, então point text pode quebrar antes de sair do canvas. Contenção continua sendo relação intencional (texto sobre placa/foto); sobreposição parcial preserva apenas a área já publicada, em vez de autorizar qualquer invasão futura. `_gInferirCorredores` pode dar a um point text uma caixa **transitória** (`_layoutW/_layoutDx`) somente no clone renderizado: o template não é reescrito, mas o valor real quebra linha antes do círculo de preço, CTA, foto, logo travado ou outra área relevante. Caixa dinâmica também recebe teto semântico de linhas (preço/código 2, título/produto 3, apoio 4, descrição/regulamento 6), nunca menor que o número de linhas realmente autorado. Rich text e preço dividido não são quebrados entre runs; participam da colisão e da redução proporcional. Sangria já existente não vira erro: só o aumento de overflow em relação ao baseline é bloqueado. `fRenderTemplateLayers` devolve as camadas efetivas, e foco, contorno e hit-test da prévia usam essa mesma geometria — o campo não fica clicável no lugar antigo depois de se acomodar.

**Dois pisos e emergência proporcional.** No degrau normal, `gStampPisosHierarquia` carimba `_pisoFonte` = o degrau tipográfico imediatamente menor da própria arte, ou 50% do tamanho desenhado quando esse é mais restritivo (título 92 com subtítulo 20 para em 46, não em 20). É o que impede que "diminuir" INVERTA a hierarquia — o erro silencioso, que ninguém percebe para reclamar. `_pisoLegivel` é o segundo freio: campos comerciais/títulos não descem de 2,2% do lado curto da arte; textos de apoio, de 1,35% (sempre limitado ao tamanho que o designer desenhou). Quando os culpados esgotam o degrau normal e ainda não cabe, `relaxou` reduz na mesma escala **somente o componente conectado à colisão** (culpados, obstáculo, âncoras e placa), podendo atravessar 50% até o mínimo global de 35%, mas nunca o piso de legibilidade; a proporção e a hierarquia permanecem, e tipografia sem relação não é tocada. Não coube: `estourou`/`_layoutInvalido` fica marcado e a exportação do franqueado é bloqueada com orientação para encurtar o texto ou trocar de material.
...
⚠ **O layout vivo é SÓ do lado do franqueado**: `fRenderTemplateLayers` exige `scope:'franqueado'`; qualquer chamada sem escopo cai em `designer` (fail-safe). Prévia e exportação recebem o mesmo clone resolvido. O Estúdio (`canvas.js`), a prévia do designer (`preview.js`) e o **Simular dados reais** usam `scope:'designer'` e mostram a geometria DESENHADA; não leem nem alteram a preferência do franqueado. O botão **Corrigir layout** e o solver genérico antigo foram removidos do Estúdio. O checklist continua auditando limites e a composição original, sem executar Auto-layout.

**Custo medido:** 0,8ms por render numa arte de 7 camadas com inferência; 12ms com 30 camadas encadeadas. No corpus de produção de 7 PSDs (31 pranchetas, 588 nós e 207 textos), o pior solve nominal/long/extremo foi 56ms — dentro do debounce de 110ms da digitação (`chat-input.js:295`). Regressões browser-native: `tests/auto-layout.html` (17 cenários) e `tests/psd-import.html` (4 casos de geometria de texto).
...
**`_foraDaArte` e `_layoutInvalido`.** A escada tem limite de voltas. Quando desiste, carimba as camadas que ficaram fora da prancheta e, separadamente, as que continuam invadindo uma área protegida. `gDescribeFranchiseeLayout` produz um contrato (`original|adapted|unsafe`, mudanças e IDs inválidos). Prévia insegura mostra o melhor esforço para permitir a correção; PNG, PDF, compartilhamento e lote lançam `LUMA_LAYOUT_UNSAFE` antes de criar o arquivo.
...
**Checklist do Estúdio.** Ele audita somente a geometria publicada e os limites escolhidos pelo designer. Não chama o solver do franqueado, não oferece correção automática e não persiste adaptação. A garantia final fica na prévia/exportação do franqueado.
...
- **Teste de tensão** (2026-08-29, `canvas.js` + `modules/toolbar.css`): dentro do **Simular dados reais**, uma régua de 0% (vazio) a 100% (o pior caso PERMITIDO, o mesmo `gStressValues` do checklist) que estica todos os campos juntos e diz o veredito a cada parada — verde "sai exatamente como você desenhou", laranja "o motor acomoda", vermelho "trava aqui — <campo>". Os cenários existentes davam PONTOS; faltava o caminho entre eles. O botão **Onde trava?** varre a régua de 10 em 10 e conta a história numa frase ("sai igual até 60% · o motor acomoda a partir de 70% · não trava nem no limite do campo"), levando a régua ao ponto que interessa olhar. ⚠ **Exceção consciente ao escopo do Estúdio**: a prévia do simulador continua mostrando a geometria DESENHADA, mas o veredito responde a outra pergunta — "o que o FRANQUEADO vai receber?" —, então roda o motor com `fitText` e compara pelo mesmo contrato do runtime (`gDescribeFranchiseeLayout`). Tudo em clones; não toca prancheta nem template. O campo culpado sai de `invalidIds` → `{{campo}}` → rótulo do Dado (nunca o token). Custo: 2 solves por parada, debounced fora do arrasto; a varredura são 11 paradas, só no clique.
...
**Regressão.** `tests/auto-layout.html` executa o motor real com Canvas 2D e cobre respiro contra círculo, corrente vertical, colapso de opcional, placa agrupada de PSD, quebra manual/token longo, hierarquia, isolamento do componente, falha impossível, determinismo/não mutação, isolamento do Estúdio e bloqueio tipado da exportação.
...
**7. Alternativas.** A escada continua sendo quebrar → empurrar → **apertar o respiro** → apertar entrelinha → **devolver o tracking** → encolher o menor degrau → escalar o componente, mas ela deixou de ser o único caminho. `gLayoutEscolherAlternativa` roda o MESMO `gApplyRelativeAnchors` com três políticas concorrentes — `sem-entrelinha` (preserva o ritmo, cede corpo), `entrelinha-livre` (piso 1.0, preserva a hierarquia) e `proporcional` (reduz o componente inteiro junto) — e escolhe a de menor penalidade. Desde 19/08 há uma quarta: `tracking-autoral`, que **não** devolve o tracking do motor (degrau 3.7) — devolver tracking muda a QUEBRA e às vezes a linha reflowa pior, então as duas versões são geradas e a nota decide. Empate: vence a padrão, que é a que o corpus conhece. ⚠ **Determinístico e condicional:** só gera alternativas quando a escada passou do empurrão (arte que resolve antes JÁ é a de alteração mínima) e **nunca** decide por tempo/carga — prévia e exportação precisam escolher a MESMA composição.
...
**8.1 Preço só cede por causa do próprio preço (2026-08-19).** Regra nova, e a única exceção ao "campo dinâmico é justamente onde o Auto-layout age": a escada **não aperta campo de preço por motivo alheio**. `gLayoutEhPrecoDinamico` (camada de texto que carrega ao menos um campo de preço) só entra na lista de culpados quando **ela mesma** cresceu/estourou a própria caixa — aí encolher é o que faz o "R$ 129,90" caber no selo desenhado pra ele. Arrastado por colisão de terceiros ou pela escala proporcional do componente, o preço fica no corpo publicado. Quem é preço: `dVars[].category==='preco'`/`type==='currency'` quando o metadado existe; no runtime do franqueado, a mesma heurística de nome da auto-criação (`gFieldGuessType`) — não há segunda régua. O motivo é de produto: o preço é o argumento da peça, e reduzi-lo porque o título ficou longo troca a promessa por um detalhe (medido: um preço curto saía **36% menor** por causa de um título gigante).

**O que continua valendo:** a corrente ainda empurra o preço (congelar a posição fazia o título crescido passar por cima — medido no corpus), ele segue obstáculo dos outros, a placa/selo atrás dele segue crescendo, e a validação de segurança não mudou.

⚠ **Conflito assumido com o item 6 (inversão de hierarquia).** Como o preço para de descer quando a própria caixa cabe, uma camada autorada MAIOR pode terminar menor que ele. A regra do preço vence: nas artes da marca o preço costuma ser o maior elemento. A inversão continua **pesando na nota** (`gScoreComposition`) — só deixou de reprovar no corpus (`tests/corpus-cases.js`, invariante 3, exceção explícita para campo de preço). Efeito medido no corpus, só nos cenários longo/extremo (nominal intacto): `de-por-lateral · longo` preço 56→69, `· extremo` 46→62, `promo-preco-circulo · extremo` 33→38, com título/produto cedendo ~5 pontos em troca. Nenhum veredito mudou, nada saiu da prancheta. Golden regravado (`LUMA_RECORD=1 node scripts/run-browser-tests.js corpus`).

**8.2 Degrau do respiro e último recurso do preço (2026-08-19).** Duas mudanças na escada, as duas medidas na bancada (`tests/_bancada.html`, 276 cenários) antes e depois.

**(a) Apertar o respiro antes de encolher a letra.** O respiro mínimo (`_gLayoutRespiro`: 0,8% do lado curto / 18% do corpo) é regra do MOTOR, não do desenho — e era ele que transformava "chegou perto" em colisão e disparava o encolhimento. Agora tem dois patamares: o ideal (fator 1) e o **apertado** (fator 0.5, piso duro de 2px), e a escada tenta o apertado **uma vez por solve** antes de tocar na tipografia. Se apertar já resolve, o laço sai sem reduzir nada — é o que um designer faz: fecha o vão antes de diminuir o corpo. Corredores e bordas continuam com o respiro ideal; só a checagem de colisão usa o patamar da volta.

**(b) O preço deixa de ser piso quando a escada trava.** Consequência medida da regra 8.1: o preço imóvel virava **piso de hierarquia externo** (`_pisoHierExterno`) para quem precisava descer — um título que colidia com o selo parava em 56px porque o preço estava em 56px, e a arte saía `unsafe`. O contrato do motor é claro (arte achatada é feia mas serve; arte bloqueada é inútil), então quando ninguém mais pode ceder o preço **deixa de contar como piso**: o título passa por baixo dele e o preço continua no corpo desenhado — que é exatamente o que a regra do preço quer.

**Medido (276 cenários da bancada, antes → depois das duas):** artes **bloqueadas 4 → 1** (0%; a única restante tem 1037px de texto fora da prancheta, impossível por desenho); artes que chegam com alguma redução 131 → 128; **pior redução 65,9% → 59,1%**; artes aprovadas com dano **0 → 0** (o guardião não abriu buraco). No corpus, as artes saem MAIORES: `promo-preco-circulo·extremo` título 48→53 e produto 32→38, `bloco-arejado·extremo` manchete 71→78. Nenhum veredito mudou de lado; golden regravado por máquina. ⚠ **Observação honesta:** no fixture `promo-preco-circulo` o título é autorado com a caixa passando por baixo do selo de preço — com o título maior, mais letras ficam atrás do selo. É a "franquia limitada" de sobreposição autorada funcionando como especificado (§sobreposição parcial), não um buraco novo; num template real a caixa do título não invade o selo.

**8.3 Degrau do tracking: devolver o que o motor adicionou (2026-08-19).** Fonte display (peso ≥900) não vem do PSD com tracking — é o **render** que soma 2% do corpo, para a caixa-alta não ficar apertada. Esse tracking é do motor, não do designer, então antes de reduzir a letra (que é a hierarquia dele) a escada devolve o que ela mesma somou: `letterSpacing` do clone cai para `max(0, efetivo − 2% do corpo)`, uma vez por camada. ⛔ **Nunca abaixo do valor autorado nem em tracking negativo** — colar glifo é estrago pior que um ponto de corpo; tracking que o designer escreveu só é apertado até 0, pela mesma conta.

**Escrito no `letterSpacing` do clone, de propósito.** É a propriedade que a MEDIDA (`gFitTextLayer`) e o RENDER (`fRenderTemplateLayers`) já leem — um carimbo novo (`_trackDelta`) seria uma segunda régua, a origem histórica dos bugs deste motor. Verificado com pixel: numa linha de 16 caracteres a 80px, tinta desenhada 849→825px e medida 855→830px — medida e desenho andam juntos (Δ de antialias). A régua de "é display?" repete o fallback por NOME do fit (`/black|realce/`), senão o degrau se desligava em silêncio em qualquer contexto sem o Estúdio carregado.

**Medido com a tipografia real da marca** (fixtures do corpus com `'Roboto Black'`, 18 cenários, alternativas ligadas nos dois lados): `de-por-lateral·longo` sai ~10% maior em quatro camadas (produto 58→64, preço 69→77, cupom 29→32, "de por" 23→25) e a pior redução da arte cai de 40% para 34%; `promo-preco-circulo·longo` preço 46→51. Nenhum cenário piorou, nenhum bloqueio novo. O caso em que devolver tracking atrapalhava (`bloco-arejado·extremo`, manchete 78→71 por reflow) é resolvido pela política `tracking-autoral`: a nota escolhe o candidato que preserva o corpo. ⚠ **As 4 suítes do CI não exercem este degrau** — os fixtures usam Arial (peso 700, sem tracking automático a devolver), então lá ele é inerte por construção; a medição de valor é a acima, com fonte display.
...
**Memória de ENCAIXE (2026-08-29) — a dívida acima, paga.** O caso patológico acima estourava o teto de 900ms da suíte de forma reprodutível: com a CPU freada em 4× o solve levava ~954ms e a `fuzz` ficava **vermelha** (só sob throttle — no CI, verde). A medição instrumentada mostrou onde: **198 encaixes e 1109 `measureText` num único solve**, porque a escada re-encaixa os culpados a cada volta e as 4 políticas alternativas re-rodam o solve inteiro sobre as mesmas camadas. `gFitTextLayer` é PURA, então ganhou memo (`_gFitChave` + o mesmo `_G_MEDIDA_CACHE` de medida e quebra, prefixo `F`): mesma entrada, mesmo encaixe, zero mudança de resultado — os 119 casos das 4 suítes seguem verdes e os vereditos do fuzz são idênticos (3 original / 56 adapted / 1 unsafe). O cálculo real virou `_gFitTextCalc`; a chave carrega tudo que o encaixe lê (tipografia, caixa, `_tetoFonte`, `_entrelinha`, pisos, teto de linhas, runs, texto) e a volta é uma cópia — devolver a mesma referência a dois clones deixaria um envenenar o `_fit` do outro.

| | antes | depois |
|---|---|---|
| p50 corpus (CPU normal) | 1,6ms | **0,6ms** |
| p50 fuzz (CPU normal) | 6,7ms | **1,0ms** |
| p50 fuzz (CPU 4× mais lenta) | 30,1ms | **4,6ms** |
| p95 fuzz (CPU 4× mais lenta) | 124ms | **98ms** |
| pior caso da suíte (4×) | 954ms (**vermelho**) | **709ms** (verde) |

**Hifenização pt-BR no corte duro (2026-08-29).** Quando uma palavra sozinha não cabe na largura, `pushToken` já cortava — mas no grafema que coubesse: `SUPERPROM/OÇÃO`. Agora o corte procura a **fronteira silábica** mais alta que ainda caiba **com o hífen** e sai `SUPERPRO-/MOÇÃO`. `gHifenizaPt` é um separador silábico determinístico de ~40 linhas (dígrafo inseparável, encontro consonantal com l/r, ditongo, consoante travando sílaba), sem dicionário e sem dependência. Guardas: palavra com menos de 6 letras não hifeniza, pedaço de 1 letra nunca, e só token alfabético latino — CJK, número e código continuam com o corte seco. ⚠ **Não reduz encolhimento de fonte**: depois do corte todas as linhas cabem, então o `estourou` não dispara e a escada não chegava a mexer no corpo. O ganho é de LEITURA.
