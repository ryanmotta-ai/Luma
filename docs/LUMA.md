...
**Correntes inferidas do desenho** (`_gInferirCorrentes`, só com o layout vivo ligado). `relativeAnchor` existe há tempos mas é marcado camada a camada, à mão — por isso a cascata quase nunca entrava em ação numa arte real. Agora a corrente sai do próprio desenho: bloco alinhado logo abaixo de outro (cruzamento horizontal ≥30% da faixa mais estreita) e a menos de **duas linhas** de distância (na escala tipográfica da própria arte) vira filho dele; o vizinho imediato acima ganha a paternidade. Ficam de fora: fundo, `layoutRole:'protected'`, travada, com posição travada, filha de grupo e grupo — logo e selo não são empurrados por texto. **Âncora manual do designer sempre vence a inferida.**
...
**A escada — quebrar → empurrar → encolher**, nessa ordem, e encolher é o ÚLTIMO recurso. Achado ao medir: `gSmartWrapText` tem quebra DURA de fallback (busca binária em `pushToken`), então qualquer texto cabe em qualquer caixa mais larga que um glifo e o encolhimento por largura quase nunca dispara sozinho. Com a cascata viva, o risco deixa de ser "encolheu até sumir" e passa a ser "empurrou para FORA da prancheta" — pior, porque antes os blocos só se sobrepunham. O laço fecha dentro de `gApplyRelativeAnchors` quando recebe `opts.canvas`: posiciona → se alguma tinta escapou (pé ou topo), reduz `_tetoFonte` em 8% de quem cresceu além da própria caixa → re-mede → reposiciona (máx. 12 voltas, ~6ms).

**Dois pisos.** `gStampPisosHierarquia` carimba `_pisoFonte` = o degrau tipográfico imediatamente menor da própria arte, ou 50% do tamanho desenhado quando esse é mais restritivo (título 92 com subtítulo 20 para em 46, não em 20). É o que impede que "diminuir" INVERTA a hierarquia — o erro silencioso, que ninguém percebe para reclamar. O carimbo é feito **antes** do laço de medida, para medida e desenho usarem o mesmo piso. Quando todo mundo chega ao piso e ainda não cabe, a prioridade inverte **uma vez** (`relaxou`): arte com o preço cortado é inútil, arte com hierarquia achatada é feia mas serve — e aí desce até o piso absoluto (50% do desenhado, nunca menos de 8px). Não coube nem lá: `estourou` fica marcado e o Estúdio é quem avisa.
...
**Interruptor, TRÊS em série** (`gLayoutVivoAtivo(publishMeta)`): a flag `franqueado.layout-vivo` no Controle do produto governa a rede; `publishMeta.layoutVivo` governa cada template (toggle na aba Instruções do publicar); e `gLayoutVivoOff` é a chave de **quem está olhando** — o botão **Auto-layout**, ao lado do Auto-zoom na barra da prévia ao vivo (`live-preview.js`). Os três precisam estar ligados. Padrão **desligado** nos dois primeiros — ligar reacomoda arte já publicada, então é decisão do designer daquela arte.

⚠ **O terceiro só DESLIGA.** O botão nem aparece num template onde o designer não ligou o Layout vivo (`gLayoutVivoDisponivel`, que é rede + template sem a chave de quem olha), então não existe caminho para o franqueado reacomodar uma arte que ninguém aprovou. Vale para a prévia **e** para o PNG baixado — as duas passam pelo mesmo `fRenderTemplateLayers`, e prévia que mente sobre o arquivo final é o defeito que este projeto mais evita. Fica em `localStorage` (`luma-lp-auto-layout`), igual ao Auto-zoom.

⚠ **Quem AUTORA usa `gLayoutVivoDisponivel`, não `gLayoutVivoAtivo`**: o Estúdio (`canvas.js`), a prévia do designer (`preview.js`) e o checklist (`linter.js`) mostram e auditam o que o TEMPLATE faz — a preferência de quem olha a prévia do franqueado não pode vazar para lá e mudar o veredito em silêncio. O simulador do Estúdio chama o render do franqueado, então neutraliza a chave em volta da chamada.
