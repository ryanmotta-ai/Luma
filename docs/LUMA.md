...
**Copy Fit — a versão que cabe** (`js/core/copy-fit.js`, 09/2026). Quando há bloqueio, `gCopyFitSugestoes` gera versões curtas (limpeza, unidade, forma curta, lista, tamanho, enfeite; nunca muda número, nunca some item, `{{campo}}`/HTML intactos) e a 1ª que o Local Fit aceita — medida por `gLocalFitMedidor`, com os dados e a placa da prévia — é oferecida no balão da prévia (`_fLpSyncBalao`), no diálogo "Esse texto não cabe" (`fCorrigirTextoLongo`, via `gConfirm` com `altLabel`) e no "Encurtar" do chat sem IA. Um toque troca, Desfazer volta; nada é trocado sozinho. Garantias, ranking, números e decisões pendentes em `docs/LOCAL-FIT-CONTRACT.md` §16.
...
- **Central de Ajuda** (`widgets/help-widget.js` + `css/modules/help-widget.css`, redesenhada em 26/09/2026 com o Deskfy de referência): um painel de 400px com três abas. **Início** tem saudação, **um** "Faça uma pergunta" (equipe online → pessoa; ninguém → assistente, com "Falar com a equipe" no fim), busca e as **Novidades do Luma** (`LUMA_NOVIDADES`, no código). **Mensagens** tem a conversa. **Ajuda** tem busca, "Nesta tela" (`WM_NESTA_TELA`: 2 artigos pela classe do body) e coleções por intenção (`WM_COLECOES`), com artigo curto em passos e "Isso resolveu?". A base de artigos é `LUMA_ARTICLES`, com cada frase conferida no código. `aud:'equipe'` = só `gIsAdmin` vê (é filtro de conteúdo, não segurança). `gOpenHelp`/`gOpenHelpTopic` abrem este painel em **todo** modo; o `#fhw` de `core/help.js` ficou sem porta. `core/help.js` segue com `gHelpKnowledge` (o índice que aterra a IA), o modal do Estúdio e a trilha. Tutorial → `tutOpen(id)`.
